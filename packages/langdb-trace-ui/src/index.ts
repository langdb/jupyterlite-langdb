...
import { CellList, INotebookModel, INotebookTracker, NotebookPanel } from '@jupyterlab/notebook';
...
class TraceUIWIdget extends Widget {
    ...
    _registerOutputChanges(cellModel: ICellModel) {
        if (!(cellModel.id in this._cellSlotMap)) {
            const fn = (sender: IOutputAreaModel, i: IOutputAreaModel.ChangedArgs) => this._cellMetadataChanged(sender, i.newIndex);
    ...
    async _updateCodeCell(cell: CodeCell, index: number) {
        ...
        if (outputs) {
            ...
            if (!node) {
                ...
                const widget = new PopoverWidget(trace_id as string);

                toolbar.addItem("popover", addToolbarButtonClass(widget));
                // Inserting into the parent container, so that any new output in
                // the outputArea layout won't push the toolbar down
                (cell.outputArea.parent?.layout as PanelLayout).insertWidget(0, toolbar);
            }
    ...
    _getCodeCell(outputAreaModel: IOutputAreaModel): CodeCell | null {
        for (let widget of this._panel.content.widgets) {
 ...
 */
   ...
   */
    ...
    private _cellSlotMap: {
        [id: string]: (model: IOutputAreaModel, i: IOutputAreaModel.ChangedArgs) => void;
    } = {};
...
class TraceUIWidgetExtension implements DocumentRegistry.WidgetExtension {
    ...
    createNew(panel: NotebookPanel, _context: DocumentRegistry.IContext<INotebookModel>): void | IDisposable {
